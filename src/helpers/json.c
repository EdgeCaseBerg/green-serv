...
int gs_commentToJSON(const struct gs_comment gsc, char * jsonOutput){
    ...
    json = "{\"id\" : %ld, \"type\":\"%s\",\"pin\" : %ld,\"message\" : \"%s\", \"timestamp\" : \"%s\", \"addressed\" : %s }";
    _escapeJSON(gsc.content, strlen(gsc.content), escaped);

    return sprintf(jsonOutput, json, gsc.id, gsc.cType,gsc.pinId ,escaped, gsc.createdTime, gsc.addressed ? "true" : "false");
}
...
int gs_commentToNJSON(const struct gs_comment gsc, char * jsonOutput, int jsonOutputAllocatedSize){
    ...
    jsonAddressedWritten = snprintf(jsonAddressed, sizeof jsonAddressed, "\"addressed\": %s,", gsc.addressed ? "true" : "false");
    ...
    if(jsonTimestampWritten + jsonPidIdWritten + jsonAddressedWritten + jsonIdWritten + jsonMessageWritten +jsonTypeWritten > jsonOutputAllocatedSize-1){
        fprintf(stderr, "%s\n", "gs_commentNToJSON may have returned partial JSON output due to not allocating enough memory");
        #ifdef RETURN_ON_JSON_RISK
            RETURN_ON_JSON_RISK;
        #endif
    }
    return snprintf(jsonOutput,jsonOutputAllocatedSize-1, "%s%s%s%s%s%s", jsonId,jsonType,jsonPinId,jsonAddressed,jsonMessage,jsonTimestamp);
